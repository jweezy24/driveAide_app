...
class VideoRecord(object):
    ...
    """
    def __init__(self, row, root_datapath,classes=0):
        self._data = row
        self._path = os.path.join(root_datapath, row[0])
        self.encoding = np.zeros(classes)
    ...
    def label(self) -> Union[int, List[int]]:
        ...
        # sample associated with multiple labels
            ...
            ls = [int(label_id) for label_id in self._data[3:-1]]
            enc = self.encoding.copy()
            for l in ls:
                enc[int(l)] = 1
            return enc
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def _load_image(self, directory: str, idx: int) -> Image.Image:
        if os.path.exists(os.path.join(directory, self.imagefile_template.format(idx))):
            return Image.open(os.path.join(directory, self.imagefile_template.format(idx))).convert('RGB')

        for j in range(1000):
            if idx-j >= 0 and os.path.exists(os.path.join(directory, self.imagefile_template.format(idx-j))):
                return Image.open(os.path.join(directory, self.imagefile_template.format(idx-j))).convert('RGB')

        for i in range(1000):
            if idx+i >= 0 and os.path.exists(os.path.join(directory, self.imagefile_template.format(idx+i))):
                return Image.open(os.path.join(directory, self.imagefile_template.format(idx+i))).convert('RGB')
    ...
    def _parse_annotationfile(self):
        ...
        for x in open(self.annotationfile_path):
            ...
            if int(tmp[2]) - int(tmp[1]) <= 0 or int(tmp[2]) - int(tmp[1]) < self.num_segments:
                continue

            if self.count_files(f"{self.root_path}/{tmp[0]}") < self.num_segments:
                continue

            if "#" not in x:
                ele = VideoRecord(x.strip().split(), self.root_path,classes=self.classes)
                tmp_list.append(ele)

                conversion_matrix = np.array([2**i for i in range(len(ele.label))])
                numeric_label = np.dot(ele.label,conversion_matrix)

                if numeric_label not in self.label_count.keys():
                    self.label_count[numeric_label] = 1
                else:
                    self.label_count[numeric_label] += 1

                orig_paths.append(x.strip().split()[-1])

        self.video_list = tmp_list
    ...
    def __getitem__(self, idx: int) -> Union[
        ...
        if not self.get_orig_paths:
            return self._get(record, frame_start_indices)
        else:
            return self._get(record, frame_start_indices,orig_path=self.orig_paths[idx])

    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]',orig_path:str = "") -> Union[
        Tuple[List[Image.Image], Union[int, List[int]]],
        Tuple['torch.Tensor[num_frames, channels, height, width]', Union[int, List[int]]],
    ]:
        """
        ...
        if not self.get_orig_paths:
            return ImglistToTensor.forward(images), record.label
        else:
            return ImglistToTensor.forward(images), record.label, orig_path
